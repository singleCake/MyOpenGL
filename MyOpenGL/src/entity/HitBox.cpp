...
HitBox::HitBox(const CVector& pos, const CVector& axis, float l, float w, float h) 
	: AbstractEntity(pos, CVector(0, 0, 0), axis) {
	createPrimitives(l, w, h);
}

HitBox::HitBox(const Cuboid& box) 
	: AbstractEntity(box.pos, CVector(0, 0, 0), box.axis) {
	primitives.clear();
	primitives.reserve(1);
...
void HitBox::draw(bool line) {
	if (!EntityManager::SHOW_HITBOX) return;
	Entity::draw(true);
	RenderManager::drawArrow(0.01, RED, getPos(), 
		getPos() + dynamic_cast<Cuboid*>(primitives[0].get())->u * 0.2f);
}

bool HitBox::checkCollision(const Ball& ball) const {
	return getBox()->checkCollision(ball);
}
...
bool HitBox::checkCollision(const HitBox& other, CVector& collisionPoint) const {
	return getBox()->checkCollision(*other.getBox(), collisionPoint);
}